	...
	<display:table uid="ideaListTable" keepStatus="false" name="ideas"
		...
		<display:column titleKey="idea.description">
			<a href="idea/details.do?ideaId=${row.id}">${row.description}</a>
				...
				<display:column>
							<a href="idea/preview.do?ideaId=${row.id}"><input type="button"
								class="btn btn-default"
								value="<spring:message code="idea.view"/>"
								onclick="self.location.href = idea/preview.do" />
							</a>		
		</display:column>	
		
		<display:column>
							<a href="comment/create.do?ideaId=${row.id}"><input type="button"
								class="btn btn-default"
